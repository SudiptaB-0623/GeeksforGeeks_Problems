...
class Solution
{
    ...
    Node *copyList(Node *head)
    {
        ...
        // Part 1 : Creating a duplicate node after each current node
        Node *temp = head;
        while(temp!=NULL)
        {
        ...
        // Part 2 : Assigning the random pointer
        temp = head;
        while(temp!=NULL)
        {
            Node *curr = temp->next;
            if(temp->arb != NULL)
                curr->arb = (temp->arb)->next;
            else
                curr->arb = NULL;
            temp = curr->next;
        }
        
        // Part 3 : Fixing the original LL and the copy of the LL
